...
function refreshLog() {
    $.get("/cgi-bin/log.cgi", function(data) {
        if (data.length == 0 || data == lastLogData)
            return;
        lastLogData = data;
        $("#log").html("");
        ...
        for(var i = 0;i < lines.length;i++){
          ...
          var converted = $(ansi_up.ansi_to_html(tokens[3])).unwrap().html();
          if(tokens.length > 5 && patchFile && converted !== undefined && converted.trim() == patchFile) {
            var lineNum = parseInt(tokens[4], 10) - 1;
            var logline = "<pre>    <span>" + patchFile + ":" + (lineNum + 1) + ":" + tokens[5] + " </span></pre>";
