...
function refreshLog() {
    $.get("/cgi-bin/log.cgi", function(data) {
        ...
        for(var i = 0;i < lines.length;i++){
          ...
          if(tokens.length > 5)
            converted = $(ansi_up.ansi_to_html(tokens[3])).unwrap().html();
          if(patchFile && converted !== undefined && converted.trim() == patchFile) {
          
            var lineNum = parseInt(tokens[4], 10) - 1;
            var logline = "<pre>    <span>" + patchFile + ":" + (lineNum + 1) + ":" + tokens[5].substring(0, tokens[5].length - 4) + " </span></pre>";
    ...
    socket.onmessage = function(event) {
        ...
        if(obj.type == "config") {
          patchFile = obj.data.patchFile;     
            ...
            $("#log").append("<pre>" + obj.title + "</pre><br/>");
            if(!obj.highlight) {
              $("#log").append("<pre>\t" + obj.msg + "</pre><br/>");
            } else {
              ...
              $("#log").append("<pre>\t<span>" + file + ":" + lineNum + ":" + rest + "</span></pre>").last().click(function() {
                codeMirror.focus();
                codeMirror.scrollIntoView({line: lineNum, ch:0});
                codeMirror.setCursor({line: lineNum, ch: 0})
                codeMirror.setSelection({line: lineNum, ch: 0}, {line: lineNum, ch: 200});
