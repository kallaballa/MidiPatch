...
function setControl(name, value) {
  var obj = { "type": "set-control", "name": name, "value": parseFloat(value) };
  socket.send(JSON.stringify(obj));
}

function noteOn(key,velocity) {
  var obj = { "type": "note-on", "note": key, "velocity": velocity };
  socket.send(JSON.stringify(obj));
}
...
function removeTransition(e) {
  ...
  var obj = { "type": "note-off", "note": $(e.target).data("key") };
  socket.send(JSON.stringify(obj));
...
function connect() {
    ...
    if(obj.type == "update-control") {
      $(obj.data.parent + "_" + obj.data.child).val(obj.data.value); 
        ...
        return;
        let pcm_chunks = str2ab(atob(obj.data));
        let node = audioCtx.createBufferSource();
        let buffer = audioCtx.createBuffer(2, pcm_chunks.byteLength, 44100);
        let data0 = buffer.getChannelData(0);
        let data1 = buffer.getChannelData(1);
        for (let i = 0; i < data0.length; i++) {
            data0[i] = pcm_chunks[i * 2];
            data1[i] = pcm_chunks[i * 2 + 1];
        }
        node.buffer = buffer;
        node.loop = false;
