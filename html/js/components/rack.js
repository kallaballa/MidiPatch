...
class MPRack {
    ...
    makeHTML() {
        return '<section id="rack"></section>';
    }
    
    makeRack(obj, oldControlParams) {
        ...
        for (var i = 0; i < obj.data.length; ++i) {
            rackDiv += "<div class=\"control-row\" style=\"background-color: " + MP_RACK_COLORS[Math.round((MP_RACK_COLORS.length - 1) * Math.random())] + ";\" id=\"" + obj.data[i].name + "\"><label class=\"moduleLabel\">" + obj.data[i].name + "</label>"
    ...
    updateControlList(obj) {
        ...
        if(cpStr) {
            ...
            if(!matches || firstUpdate) {
                firstUpdate = false;
                this.makeRack(obj,undefined);
            }
            else {
                var r = this;
                dialogs.askControlParametersMatch(function() {
                    r.makeRack(obj,oldControlParams);
                },function() {
                    r.makeRack(obj,undefined);
                });
            }
        } else {
            this.makeRack(obj,undefined);
