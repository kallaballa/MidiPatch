...
local vibrato = SineWave():freq(vibratoFreq * 10.0);
...
local env = ADSR()
:attack(synth:addParameter("attack",0))
:decay(synth:addParameter("decay", 0 ))
:sustain(synth:addParameter("sustain",1))
:release(synth:addParameter("release",0))
:doesSustain(1)
:trigger(gate);

synth:setOutputGen((tone * env * ( FixedValue(1.0) + (tremolo * (tremoloAmount * 3)) )) * (FixedValue(0.02) + noteVelocity * 0.005))
